        ...
        <Table hover responsive className="mb-0" style={{ borderCollapse: "separate", borderSpacing: "0" }}>
          <thead style={{ background: "#f8fafc", borderBottom: "2px solid #f1f5f9" }}>
            <tr>
              ...
              <th className="px-4 py-3 border-0 text-center">Borrow Days</th> {/* Đổi từ Request Date */}
              ...
              [...history].reverse().map(record => {
                ...
                return (
                  <tr key={record.id} style={{ verticalAlign: "middle" }}>
                    ...
                    <td className="px-4 py-4 border-0 border-bottom text-center">{getBorrowDays(record)}</td> {/* Hiển thị số ngày mượn */}
